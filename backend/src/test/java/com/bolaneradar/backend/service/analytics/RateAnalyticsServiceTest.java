...
/**
 * Enhetstester för RateAnalyticsService.
 * <p>
 * Fokus:
 *  - Filtrering och sortering av historik
 *  - Trend-beräkningar mellan datum
 *  - Val av rätt "läge" beroende på parametrar
 */
...
class RateAnalyticsServiceTest {
    ...
    // ========== Hjälpmetod för att skapa MortgageRate ==========
    ...
    void getRateHistoryForBank_filtersByDateTypeTerm_andSortsAscByDefault() {
        ...
        MortgageRate r3 = rate(bank, MortgageTerm.FIXED_3Y, RateType.LISTRATE, d3, 3.8); // annan term

        when(mortgageRateRepository.findByBank(bank))
                .thenReturn(List.of(r3, r1, r2)); // osorterat med flit

        List<MortgageRate> result = rateAnalyticsService.getRateHistoryForBank(
                bank,
                LocalDate.of(2024, 1, 1),
                LocalDate.of(2024, 1, 15),
                null,                       // sort = default (asc)
                RateType.LISTRATE,
    ...
    // ============================================================
    @Test
    void getAllBanksRateHistory_returnsMapPerBank_andDelegatesToGetRateHistoryForBank() {
        ...
        Map<String, List<MortgageRate>> result = rateAnalyticsService.getAllBanksRateHistory(
                List.of(seb, nordea),
                null,
                null,
                "asc"
    ...
    void getRateTrends_withExplicitFromTo_buildsTrendsBetweenTwoDates() {
        ...
        when(mortgageRateRepository.findByEffectiveDate(from))
        ...
        assertEquals("FIXED_1Y", t.getTerm());
        assertEquals("LISTRATE", t.getRateType());
        assertEquals(3.0, t.getPreviousRate());
        assertEquals(3.4, t.getCurrentRate());
        assertEquals(from, t.getFromDate());
        assertEquals(to, t.getToDate());
        assertEquals(0.4, t.getChange());
    }

    // ============================================================
    // getRateTrends() – AVERAGERATE per bank (utan from/to)
    // ============================================================
    @Test
    void getRateTrends_forAverageRate_usesPerBankDatesAndBuildsTrends() {
        ...
        assertEquals(1, trends.size());
        RateTrend t = trends.getFirst();
        assertEquals("SEB", t.getBankName());
        assertEquals("VARIABLE_3M", t.getTerm());
        assertEquals("AVERAGERATE", t.getRateType());
        assertEquals(3.1, t.getPreviousRate());
        assertEquals(3.5, t.getCurrentRate());
        assertEquals(prevDate, t.getFromDate());
        assertEquals(latestDate, t.getToDate());
    }

    // ============================================================
    // getRateTrends() – globala datum (LISTRATE-läge)
    // ============================================================
    @Test
    void getRateTrends_globalMode_usesTwoLatestDatesAndFiltersByType() {
        ...
        MortgageRate prevAverage = rate(seb, MortgageTerm.FIXED_3Y, RateType.AVERAGERATE, prevDate, 3.8); // ska filtreras bort

        MortgageRate latestListRate = rate(seb, MortgageTerm.FIXED_3Y, RateType.LISTRATE, latestDate, 4.2);
        MortgageRate latestAverage = rate(seb, MortgageTerm.FIXED_3Y, RateType.AVERAGERATE, latestDate, 4.0); // ska filtreras bort
        ...
        RateTrend t = trends.getFirst();
        assertEquals("SEB", t.getBankName());
        assertEquals("FIXED_3Y", t.getTerm());
        assertEquals("LISTRATE", t.getRateType());
        assertEquals(4.0, t.getPreviousRate());
        assertEquals(4.2, t.getCurrentRate());
    }

    // ============================================================
    // getRateTrendsInRange()
    // ============================================================
    @Test
    void getRateTrendsInRange_buildsTrendsForConsecutiveDatesPerGroup_andSortsByChangeDesc() {
        ...
        // Sortering: samma bank, term, typ → sorteras på change (fallande)
        RateTrend first = trends.get(0);
        RateTrend second = trends.get(1);

        assertTrue(first.getChange() >= second.getChange());
        assertEquals("SBAB", first.getBankName());
        assertEquals("SBAB", second.getBankName());
        assertEquals("FIXED_1Y", first.getTerm());
        assertEquals("LISTRATE", first.getRateType());
    }
}